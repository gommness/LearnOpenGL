...
vec3 computeDirectionalLight(DirectionalLight light, Material material, vec2 texCoords, vec3 normal, vec3 viewDir){
    ...
    //float diffuseAlike = clamp(smoothstep(0, 0.2,dot(lightDir, normal)), 0.2, 1.0);
    //float specularAlike = clamp(smoothstep(0, 0.6,dot(reflectDir, viewDir)), 0.2, 1.0);
    float diffuseAlike = smoothstep(0, 0.2,dot(lightDir, normal));
    float specularAlike = 1;//smoothstep(0, 0.2,dot(reflectDir, viewDir));
    //return vec3(max(diffuseAlike, specularAlike));
    return 0.25*vec3(specularAlike*diffuseAlike);//*vec3(texture(material.texture_diffuse1, texCoords));
}
...
vec3 computePointLight(PointLight light, Material material, vec2 texCoords, vec3 normal, vec3 viewDir, vec3 fragPos){
    ...
    vec3 reflectDir = reflect(-lightDir, normal);
    float diffuseAlike = smoothstep(0, 0.2,dot(lightDir, normal));
    float specularAlike = 1;//step(0.99,dot(normal, viewDir));
    //return vec3(max(diffuseAlike, specularAlike));
    return vec3(specularAlike*diffuseAlike);//*vec3(texture(material.texture_diffuse1, texCoords));
}
...
void main(){
    ...
    for(int i=0; i<N_POINTS; ++i){
        result += computePointLight(pointLight[i], material, fragTexCoords, fragNormal, viewDir, fragPos)/(N_POINTS+1);
    }
