...
using Strip = std::vector<Point>;
using Generatrix = std::function<Point(Point&)>;

Strip generateLine(Point start, Point end, size_t n_iter) {
    Strip strip;
    ...
    double incr = 1.0/n_iter;
    double acc = 0;

    for (int i = 0; i < n_iter; i++, acc+=incr) {
        float x = std::lerp(xStart, xEnd, acc);
        float y = std::lerp(yStart, yEnd, acc);
        float z = std::lerp(zStart, zEnd, acc);
        strip.push_back(Point{x, y, z});
    }
    return strip;
}
